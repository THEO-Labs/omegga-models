"""Stage 7 BBD HNM v2 — single-class sigmoid CNN with hard-negative training.

Mirrors omegga-ml-training/scripts/train_bbd_v2.py inference recipe:
...
- inference: P(defect) = sigmoid(logit)
"""
...
_DEFAULT_CKPT = _HERE / "bbd_hnm_v2.pth"
_BACKBONE = "tf_efficientnet_b0.ns_jft_in1k"

# Threshold from sweep on v2 test set (best_auc model, AUC 0.9011):
#   t=0.330  R=80.7%  P=10.9%  <- balanced production candidate
#   t=0.229  R=90.7%  P=4.3%   <- high recall
#   t=0.397  R=70.2%  P=16.9%  <- conservative
THRESHOLD = 0.330
...
def _preprocess(img_bgr: np.ndarray):
    ...
    resized = cv2.resize(rgb_norm, (_INPUT_SIZE, _INPUT_SIZE), interpolation=cv2.INTER_LINEAR)
